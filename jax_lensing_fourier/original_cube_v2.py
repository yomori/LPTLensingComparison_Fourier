...
#out_dim = [jnp.int32(jnp.ceil(box_size[1])),jnp.int32(jnp.ceil(box_size[2])),jnp.int32(jnp.ceil(box_size[0]))]
#print(out_dim)
...
def make_full_field_model(field_size, cube_size, cubegrid_size, box_size, out_dim, transform, method='lpt',density_plane_width=None, density_plane_npix=None):
  
    def density_plane_fn(t, y, args):

        cosmo, _ , density_plane_width, density_plane_npix,cc  = args

        density_plane_npix_x = density_plane_npix[0]
        density_plane_npix_y = density_plane_npix[1]
        ...
        # Note -- position here is in terms of grid size not box size so convert to comoving size
        positions = transform(positions/cubegrid_size)*cube_size
        

        x      = positions[..., 1]/box_size[1] * density_plane_npix_x
        y      = positions[..., 2]/box_size[2] * density_plane_npix_y
        d      = positions[..., 0]

        #now in density plane coordinates
        ...
        density_plane = cic_paint_2d(jnp.zeros([density_plane_npix_x, density_plane_npix_y]), jnp.c_[x,y], weight)

        # Normalization: normalized by the effective grid volume
        # densityplane = M/densitypixL**2
        # (density_plane_npix_x*density_plane_npix_y) = densitypixL**2
        # cubegrid_size**3 * density_plane_width / box_size[0] = effective grid volume
        density_plane = density_plane / (cubegrid_size**3/(density_plane_npix_x*density_plane_npix_y) * density_plane_width / box_size[0] )
    ...
    def forward_model(cosmo, nz_shear, lin_field):
        ...
        if method=='lpt':
            ...
            r_center = (jnp.arange(grid_size[-1]) + 0.5)*box_size[-1]/grid_size[-1]
            a_center = jc.background.a_of_chi(cosmo, r_center)
            a        = a_center

            # Compute displacement and paint positions of particles onto lightcone
            eps,_     = lpt_lightcone(cosmo, lin_field, particles, a_center, grid_size)
